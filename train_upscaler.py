...
# Hard-patch mining: after this many epochs, examples with high
# student-vs-teacher error get sampled more often.
HARD_START_EPOCH = 15
HARD_REFRESH     = 5
HARD_TOP_FRAC    = 0.25
HARD_BOOST       = 3.0
...
class CachedPatchDataset(Dataset):
    def __init__(self, cache_file):
        ...
        # Mutable sampling weights for hard-example mining.
        self.sample_weights = torch.ones(self.n, dtype=torch.float32)
    ...
    for lr, hr, _ in loader:
        ...
        for out, accumulator in (
            ...
            if accumulator == "student":
                sum_psnr += psnr.sum().item()
            elif accumulator == "teacher":
                sum_teacher_psnr += psnr.sum().item()
            else:
                sum_bicubic_psnr += psnr.sum().item()
...
# ============================================================
@torch.no_grad()
def refresh_hard_weights(student, teacher, dataset, batch_size=256):
    student.eval()
    teacher.eval()
    ...
    errors = torch.zeros(len(dataset), dtype=torch.float32)

    for lr, hr, indices in loader:
        lr = lr.to(device, non_blocking=True)
        hr = hr.to(device, non_blocking=True)
        ...
        # Mine based on teacher disagreement + edge error.
        diff = torch.mean(torch.abs(pred - teacher_out), dim=(1, 2, 3))
        edge = torch.mean(
            torch.abs(sobel_grad(pred) - sobel_grad(hr)),
            dim=(1, 2, 3),
        )

        score = diff + 0.5 * edge
        errors[indices] = score.cpu()

    threshold = torch.quantile(errors, 1.0 - HARD_TOP_FRAC)
    hard = errors >= threshold

    weights = torch.ones_like(errors)
    weights[hard] = HARD_BOOST

    dataset.sample_weights = weights

    print(
        f"Hard mining: {hard.sum().item()} / {len(dataset)} patches "
        f"({100.0 * hard.float().mean().item():.1f}%) boosted x{HARD_BOOST}"
    )
...
def train():
    ...
    # Deterministic subset for training validation.
    n_val = min(VAL_PATCHES, len(dataset))
    val_subset = Subset(dataset, list(range(n_val)))
    ...
    print("\n--- Training student ---")
    print(
        "Loss = HR pixel + edge + high-frequency + teacher residual distillation"
    )
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if (
            epoch >= HARD_START_EPOCH
            and (epoch - HARD_START_EPOCH) % HARD_REFRESH == 0
        ):
            refresh_hard_weights(student, teacher, dataset)
