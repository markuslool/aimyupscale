...
PATCHES_PER_IMAGE_CACHE = 80                    # ~64k патчей всего (~1 GB RAM)
...
LEARNING_RATE   = 1e-3
USE_AMP         = True
PATCHES_PER_IMG = 30                            # виртуальная длина эпохи (не влияет на кэш)
VAL_EVERY       = 5
EMA_START_EPOCH = 15
...
class MiniESPCN(nn.Module):
    ...
    Receptive field: 9x9. Параметров: ~13K.
    """
...
def train():
    ...
    # Валидация — 200 случайных патчей из того же кэша (быстро, без I/O)
    n_val = min(200, len(dataset))
    val_subset = Subset(dataset, indices=list(range(n_val)))
    ...
    print(f"Патчей: {len(dataset)} | Батчей на эпоху: {len(dataloader)} | "
          f"Шагов всего: {len(dataloader) * EPOCHS}")
    model.train()
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if epoch % VAL_EVERY == 0 or epoch == EPOCHS:
            ...
            if score > best_psnr:
                best_psnr = score
                torch.save({
                    'model': ema_model.state_dict() if psnr_ema is not None
